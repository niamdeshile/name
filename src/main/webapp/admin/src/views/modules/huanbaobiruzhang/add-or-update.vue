          ...
          <el-form-item class="input" v-if="ruleForm.riqi" label="日期" prop="riqi">
              <el-input v-model="ruleForm.riqi" 
                placeholder="日期" readonly></el-input>
                                                                                ...
                                                                                <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="入账原因" prop="ruzhangyuanyin">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="入账原因"
                  v-model="ruleForm.ruzhangyuanyin" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.ruzhangyuanyin" label="入账原因" prop="ruzhangyuanyin">
                    <span>{{ruleForm.ruzhangyuanyin}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.huanbaobi = true;
            continue;
	            	            ...
	            	            if(o=='ruzhangyuanyin'){
            this.ruleForm.ruzhangyuanyin = obj[o];
	    this.ro.ruzhangyuanyin = true;
            continue;
          }
	            	            if(o=='riqi'){
    ...
    onSubmit() {
      ...
      obj.huanbaobi = parseFloat(obj.huanbaobi) + parseFloat(this.ruleForm.huanbaobi)
      this.$http({
          url: `${table}/update`,
          method: "post",
          data: obj
        ...
        if (valid) {
          this.$http({
            url: `huanbaobiruzhang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huanbaobiruzhangCrossAddOrUpdateFlag = false;
                  this.parent.search();
