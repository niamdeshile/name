<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="环保类型" prop="huanbaoleixing">
          <el-input v-model="ruleForm.huanbaoleixing" 
              placeholder="环保类型" clearable  :readonly="ro.huanbaoleixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `huanbaofenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huanbaofenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
