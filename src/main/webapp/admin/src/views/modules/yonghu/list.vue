<template>
  <div class="main-content">
		    ...
		    label="邮箱">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('yonghu','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('yonghu','环保币') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yonghuhuanbaobiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'环保币':'' }}</el-button>
                <el-button v-if="isAuth('yonghu','环保币') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yonghuhuanbaobiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'环保币':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yonghu','环保币') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yonghuhuanbaobiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'环保币':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <yonghuhuanbaobi-cross-add-or-update v-if="yonghuhuanbaobiCrossAddOrUpdateFlag" :parent="this" ref="yonghuhuanbaobiCrossaddOrUpdate"></yonghuhuanbaobi-cross-add-or-update>
...
import yonghuhuanbaobiCrossAddOrUpdate from "../yonghuhuanbaobi/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                                                    searchForm: {
      ...
      addOrUpdateFlag:false,
                  yonghuhuanbaobiCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
            yonghuhuanbaobiCrossAddOrUpdate,
          },
  methods: {
    contentStyleChange() {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            yonghuhuanbaobiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yonghuhuanbaobiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yonghu');
      this.$nextTick(() => {
        this.$refs.yonghuhuanbaobiCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                                                                                                                                                                                            },
